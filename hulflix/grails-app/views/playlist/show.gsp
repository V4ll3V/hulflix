	...
	<body>
		...
		<div id="show-playlist" class="content scaffold-show" role="main">
			...
			<ol class="property-list playlist">
				...
				<g:if test="${playlistInstance?.playlistentry}">
				<li class="fieldcontain">
					<span id="playlistentry-label" class="property-label"><g:message code="playlist.playlistentry.label" default="Playlistentry" /></span>
					
						<g:each in="${playlistInstance.playlistentry}" var="p">
						<span class="property-value" aria-labelledby="playlistentry-label"><g:link controller="playlistentry" action="show" id="${p.id}">${p?.encodeAsHTML()}</g:link></span>
						</g:each>
